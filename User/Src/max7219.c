...
void MAX7219_Init(void) {
  ...
  MAX7219_ControlAll(TEST, OFF);                   // no test
  MAX7219_ControlAll(SCANLIMIT, ROW_SIZE - 1);       // scan limit is set to max on startup
  MAX7219_ControlAll(INTENSITY, MAX_INTENSITY / 2);  // set intensity to a reasonable value
  MAX7219_ControlAll(DECODE, OFF);                 // ensure no decoding (warm boot potential issue)
  MAX7219_ClearAll();
  MAX7219_ControlAll(SHUTDOWN, OFF);               // take the modules out of shutdown mode
}
...
static void MAX7219_SpiSend() {
  CS_SET();
  HAL_SPI_Transmit(&hspi1, _spiData, SPI_DATA_SIZE, HAL_MAX_DELAY);
  CS_RESET();
}

static void MAX7219_SpiClearBuffer(void) {
  memset(_spiData, 0, SPI_DATA_SIZE);
}

static void MAX7219_FlushBufferOne(uint8_t buf)
...
static void MAX7219_FlushBufferAll()
...
static void MAX7219_ControlHardware(uint8_t dev, controlRequest_t mode, int value) {
...
bool MAX7219_ControlOne(uint8_t buf, controlRequest_t mode, int value) {
  ...
  if (mode < UPDATE) {
    ...
    MAX7219_SpiClearBuffer();
    MAX7219_ControlHardware(buf, mode, value);
    MAX7219_SpiSend();
  } else {
...
bool MAX7219_ControlBy(uint8_t startDev, uint8_t endDev, controlRequest_t mode, int value) {
  ...
  if (mode < UPDATE) {
    ...
    for (uint8_t i = startDev; i <= endDev; i++) {
      MAX7219_ControlHardware(i, mode, value);
    }
...
void MAX7219_ControlAll(controlRequest_t mode, int value) {
  MAX7219_ControlBy(0, MAX7219_GetDeviceCount() - 1, mode, value);
}
...
void MAX7219_ClearAll(void) {
  MAX7219_ClearBy(0, MAX7219_GetDeviceCount() - 1);
}

uint8_t MAX7219_BitReverse(uint8_t b)
...
uint8_t MAX7219_GetC(uint8_t buf, uint8_t c)
...
uint8_t MAX7219_GetR(uint8_t buf, uint8_t r)
...
// returned value is in pixel coordinates
{
  ...
  uint8_t value = _hwRevCols ? MAX7219_BitReverse(_matrix[buf].dig[HW_ROW(r)]) : _matrix[buf].dig[HW_ROW(r)];

  return (value);
}

bool MAX7219_CopyC(uint8_t buf, uint8_t cSrc, uint8_t cDest)
...
bool MAX7219_CopyR(uint8_t buf, uint8_t rSrc, uint8_t rDest)
// Src and Dest are in pixel coordinates.
...
bool MAX7219_CopyColumn(uint8_t buf, uint8_t cSrc, uint8_t cDest) {
  if (_hwDigRows)
    return (MAX7219_CopyC(buf, cSrc, cDest));
  else
    return (MAX7219_CopyR(buf, cSrc, cDest));
}

bool MAX7219_CopyRow(uint8_t buf, uint8_t rSrc, uint8_t rDest) {
  if (_hwDigRows)
    return (MAX7219_CopyR(buf, rSrc, rDest));
  else
    return (MAX7219_CopyC(buf, rSrc, rDest));
}

uint8_t MAX7219_GetDevColumn(uint8_t buf, uint8_t c) {
  if (_hwDigRows)
    return (MAX7219_GetC(buf, c));
  else
    return (MAX7219_GetR(buf, c));
}

uint8_t MAX7219_GetPixelColumn(uint8_t c) {
  return MAX7219_GetDevColumn((c / COL_SIZE), c % COL_SIZE);
}

uint8_t MAX7219_GetRow(uint8_t buf, uint8_t r) {
  if (_hwDigRows)
    return (MAX7219_GetR(buf, r));
  else
    return (MAX7219_GetC(buf, r));
}

bool MAX7219_SetC(uint8_t buf, uint8_t c, uint8_t value)
...
bool MAX7219_SetR(uint8_t buf, uint8_t r, uint8_t value)
// r and value are in pixel coordinates
{
  if ((buf > LAST_BUFFER) || (r >= ROW_SIZE))
    return (false);

  _matrix[buf].dig[HW_ROW(r)] = _hwRevCols ? MAX7219_BitReverse(value) : value;
...
bool MAX7219_SetDevColumn(uint8_t buf, uint8_t c, uint8_t value) {
  if (_hwDigRows)
    return (MAX7219_SetC(buf, c, value));
  else
    return (MAX7219_SetR(buf, c, value));
}

bool MAX7219_SetPixelColumn(uint16_t c, uint8_t value) {
  return MAX7219_SetDevColumn((c / COL_SIZE), c % COL_SIZE, value);
}

bool MAX7219_SetRowOne(uint8_t buf, uint8_t r, uint8_t value) {
  if (_hwDigRows)
    return (MAX7219_SetR(buf, r, value));
  else
    return (MAX7219_SetC(buf, r, value));
}

bool MAX7219_TransformBuffer(uint8_t buf, transformType_t ttype) {
  uint8_t t[ROW_SIZE];
  ...
  case TFLR: // Transform Flip Left to Right
    if (_hwDigRows) {
      for (uint8_t i = 0; i < ROW_SIZE; i++)
        _matrix[buf].dig[i] = MAX7219_BitReverse(_matrix[buf].dig[i]);
    } else  // really a TFUD
    {
  ...
  case TRC: // Transform Rotate Clockwise
    for (uint8_t i = 0; i < ROW_SIZE; i++)
      t[i] = MAX7219_GetDevColumn(buf, COL_SIZE - 1 - i);

    for (uint8_t i = 0; i < ROW_SIZE; i++)
      MAX7219_SetRowOne(buf, i, t[i]);
    break;
...
bool MAX7219_TransformOne(uint8_t buf, transformType_t ttype) {
  ...
  if (!MAX7219_TransformBuffer(buf, ttype))
    return (false);
...
bool MAX7219_GetBuffer(uint16_t col, uint8_t size, uint8_t *pd) {
  if ((col >= MAX7219_GetColumnCount()) || (pd == NULL))
    return (false);

  for (uint8_t i = 0; i < size; i++)
    *pd++ = MAX7219_GetPixelColumn(col--);
...
bool MAX7219_SetRowAll(uint8_t r, uint8_t value) {
  return MAX7219_SetRowBy(0, MAX7219_GetDeviceCount() - 1, r, value);
}
...
bool MAX7219_TransformBy(uint8_t startDev, uint8_t endDev, transformType_t ttype) {
  ...
  case TSL: // Transform Shift Left one pixel element (with overflow)
    colData = 0;
    if (_wrapAround)
      colData = MAX7219_GetPixelColumn(((endDev + 1) * COL_SIZE) - 1);

    // shift all the buffers along
    for (int8_t buf = endDev; buf >= startDev; --buf) {
      MAX7219_TransformBuffer(buf, ttype);
      // handle the boundary condition
      MAX7219_SetDevColumn(buf, 0, MAX7219_GetDevColumn(buf - 1, COL_SIZE - 1));
    }

    MAX7219_SetPixelColumn((startDev * COL_SIZE), colData);
    break;
  ...
  case TSR: // Transform Shift Right one pixel element (with overflow)
    ...
    colData = 0;
    if (_wrapAround)
      colData = MAX7219_GetPixelColumn(startDev * COL_SIZE);
    ...
    for (uint8_t buf = startDev; buf <= endDev; buf++) {
      ...
      // handle the boundary condition
      MAX7219_SetDevColumn(buf, COL_SIZE - 1, MAX7219_GetDevColumn(buf + 1, 0));
    }

    MAX7219_SetPixelColumn(((endDev + 1) * COL_SIZE) - 1, colData);
    break;
...
bool MAX7219_TransformAll(transformType_t ttype){
  return MAX7219_TransformBy(0, MAX7219_GetDeviceCount()-1, ttype);
}
...
void MAX7219_UpdateMode(controlValue_t mode) {
  MAX7219_ControlAll(UPDATE, mode);
}
...
void MAX7219_Wraparound(controlValue_t mode) {
  MAX7219_ControlAll(WRAPAROUND, mode);
}
