  ...
  for (uint8_t row = 0; row < 8; row++) {
    for (uint8_t dev = 0; dev < MAX_DEVICES; dev++) {
      ...
      switch (dev) {
        case 0:
          data =
              numbers_5x8[hour_1st][row] | ((numbers_5x8[hour_2nd][row]) >> 5);
          break;
        case 1:
          data = numbers_5x8[hour_2nd][row] << 3 | (signs[0][row] >> 3) |
...
static void led_display_date(uint8_t month, uint8_t day_of_month,
  ...
  for (uint8_t row = 0; row < 8; row++) {
    for (uint8_t dev = 0; dev < MAX_DEVICES; dev++) {
      ...
      switch (dev) {
        case 0:
          data = numbers_5x8[month_1st][row] |
                 ((numbers_5x8[month_2nd][row]) >> 5);
          break;
        case 1:
        ...
        case 2:
          data = (numbers_5x8[day_1st][row] << 1) |
                 (numbers_5x8[day_2nd][row] >> 4);
          break;
        case 3:
          data =
              (numbers_5x8[day_2nd][row] << 4) |
              ((numbers_3x5[day_of_week == 1 ? 7 : day_of_week - 1][row]) >> 5);
          break;
      }
      MAX72XX_SetRowOne(dev, row, data);
    }
...
// static void modify_frame_data(uint8_t *data, uint8_t row, uint8_t col, bool
// bit) {
...
//                const uint8_t unit_row_byte = *(row_frame_data + row *
//                LED_UNIT_NUM + unit); uint8_t *col_data = &col_frame_data[row
//                * unit];
...
// static uint8_t* merge_sec_data(uint8_t *time_frame_data_from, uint8_t
// from_begin_row, uint8_t from_len, uint8_t *time_frame_data_to, uint8_t
// to_begin_row, uint8_t to_len) {
//    if ((one_len + two_len) > 8) {
//        return NULL;
//    }
//
//    for (int8_t i = 0; i < one_len; i++) {
//        const uint8_t row_data = *(time_frame_data_from + (one_begin_row + i)
//        * LED_UNIT_NUM + 3); merge_data[i] = *row_data;
//    }
//
//    for (int8_t i = 0; i < two_len; i++) {
//        const uint8_t row_data = *(time_frame_data_two + (two_begin_row + i) *
//        LED_UNIT_NUM + 3); merge_data[i + one_len + 1] = *row_data;
//    }
//
//    return merge_data;
//}
...
void Clock_ShowTime(void) { led_display_time(rtc.Hour, rtc.Min, rtc.Sec); }

void Clock_ShowDate(void) {
  led_display_date(rtc.Month, rtc.Day, rtc.DaysOfWeek);
}

void Clock_SecondJumpUp(void) { MAX72XX_TransformOne(3, TSU); }

void Clock_SecondJumpDown(void) { MAX72XX_TransformOne(3, TSD); }
